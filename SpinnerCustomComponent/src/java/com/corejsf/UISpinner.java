...
public class UISpinner extends UIInput {
    ...
    private void encodeInputField(ResponseWriter writer, String clientId)
        ...
        writer.writeAttribute("style", "font-size:20px;font-family:verdana;color:red", "label");
    ...
    private int getIncrementedValue(int submittedValue, int increment) {
        ...
        if ((minimum == null || newValue >= minimum.intValue())&& (maximum == null || newValue <= maximum.intValue())) {
            ...
            Integer retVal=submittedValue;
            if(newValue < minimum.intValue()){
                retVal= minimum;
            }
            if(newValue >maximum.intValue()){
                retVal= maximum;
            }
            return retVal;
        }
